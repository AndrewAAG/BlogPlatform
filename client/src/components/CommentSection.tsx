...
const CommentSection = ({ postId }: CommentSectionProps) => {
    ...
    const handleEdit = (comment: Comment) => {
        ...
        setReplyingTo(null); // Close reply if open
    };
                ...
                <div key={comment.id} className={`mt-6 ${isReply ? 'ml-8 pl-4 border-l-2 border-slate-100' : ''}`}>
                    <div className="flex gap-3">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center justify-between mb-1">
                                ...
                                {isOwner && editingId !== comment.id && (
                                    <div className="flex items-center gap-2 text-slate-400">
                                        <button onClick={() => handleEdit(comment)} className="hover:text-primary-600 transition-colors">
                                            <Edit2 size={14} />
                                        </button>
                                        <button onClick={() => handleDelete(comment.id)} className="hover:text-red-500 transition-colors">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                )}
                            ...
                            {replyingTo === comment.id && !editingId && (
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 border-t border-slate-100 mt-10">
            ...
            {deleteId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 transform scale-100 animate-in zoom-in-95 duration-200">
